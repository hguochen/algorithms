...
public class BinaryMaxHeap {
	...
	public ArrayList<Integer> heapify(ArrayList<Integer> newArrayList) {
	...
	private static ArrayList<Integer> siftDown(ArrayList<Integer> newArrayList, int pos, int arraySize) {
		...
		while (2 * pos + 1 <= maxIndex) { // while there is a left node in heap subtree
			...
			// if child index is still within scope AND right child value is bigger than left child value
				child_index += 1;
			} // child_index is now the bigger contained value of the 2 subtrees

			if (newArrayList.get(child_index) > temp) { // child_index value is bigger than newArrayList root value
